...
public class StorageWithGeneration {
	...
	private static boolean includeOld = true;
	...
	public static void delete(Collectable obj) {
		if (youngGeneration.remove(obj)) {
			log.append("Delete from young generation: " + obj.toString() + "\n");
		} else if (oldGeneration.remove(obj)){
			log.append("Delete from old generation: " + obj.toString() + "\n");
		} else {
			log.append(
	...
	private static void copyYoungToOld() {
		for(Collectable cObject : getYoungGeneration()) {
